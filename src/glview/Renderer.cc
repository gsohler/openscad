...
void Renderer::setupShader() {
  ...
  renderer_shader_.data.color_rendering.tex1 = glGetUniformLocation(shader_prog, "tex1"); //4 
  renderer_shader_.data.color_rendering.texturefactor = glGetUniformLocation(shader_prog, "textureFactor"); // 5
  ...
  if(textures.size() > 0) { // TODO prevent bug, segfault
    glUniform1f(renderer_shader_.data.color_rendering.texturefactor , 0.0);
    glUniform1i(renderer_shader_.data.color_rendering.tex1 , 0);
  }
...
void Renderer::setColor(const float color[4], const int &textureind, const RendererUtils::ShaderInfo *shaderinfo) const
{
  ...
  if (shaderinfo) {
    ...
    if(textureind == 0){
    	    glUniform1f(shaderinfo->data.color_rendering.texturefactor, 0.0);
    } else {
            glBindTexture(GL_TEXTURE_2D, textureIDs[textureind-1]);
    	    glUniform1f(shaderinfo->data.color_rendering.texturefactor, 1.0);
    }
  }
#endif
}
...
Color4f Renderer::setColor(ColorMode colormode, const float color[4], const int &textureind, const RendererUtils::ShaderInfo *shaderinfo) const
{
  PRINTD("setColor b");
  Color4f basecol;
