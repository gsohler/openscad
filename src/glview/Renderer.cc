  ...
  if (ps.getDimension() == 2) {
    ...
    for (double z : {-zbase / 2, zbase / 2}) {
      for (const auto& poly : ps.indices) {
        if (poly.size() == 3) {
          if (z < 0) {
            gl_draw_triangle(shaderinfo, ps.vertices[poly.at(0)], ps.vertices[poly.at(2)], ps.vertices[poly.at(1)], true, true, true, z, mirrored, textureind);
          } else {
            gl_draw_triangle(shaderinfo, ps.vertices[poly.at(0)], ps.vertices[poly.at(1)], ps.vertices[poly.at(2)], true, true, true, z, mirrored, textureind);
          }
        } else if (poly.size() == 4) {
          if (z < 0) {
            gl_draw_triangle(shaderinfo, ps.vertices[poly.at(0)], ps.vertices[poly.at(3)], ps.vertices[poly.at(1)], false, true, true, z, mirrored, textureind);
            gl_draw_triangle(shaderinfo, ps.vertices[poly.at(2)], ps.vertices[poly.at(1)], ps.vertices[poly.at(3)], false, true, true, z, mirrored, textureind);
          } else {
            gl_draw_triangle(shaderinfo, ps.vertices[poly.at(0)], ps.vertices[poly.at(1)], ps.vertices[poly.at(3)], false, true, true, z, mirrored, textureind);
            gl_draw_triangle(shaderinfo, ps.vertices[poly.at(2)], ps.vertices[poly.at(3)], ps.vertices[poly.at(1)], false, true, true, z, mirrored, textureind);
          }
        } else {
...
std::vector<SelectedObject> Renderer::findModelObject(Vector3d near, Vector3d far, int mouse_x, int mouse_y, double tolerance) { return std::vector<SelectedObject>(); }
#else //NULLGL
