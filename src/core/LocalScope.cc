...
void LocalScope::print_python(std::ostream& stream, std::ostream& stream_def, const std::string& indent, const bool inlined, const bool skip_brackets) const
{
  ...
  for (const auto& assignment : this->assignments) {
    assignment->print_python(tmpstream, stream_def, indent);
  }
  if(this->moduleInstantiations.size() == 1) {
    this->moduleInstantiations[0]->print_python(stream, stream_def, indent, inlined, skip_brackets);
  } else {
    if(!skip_brackets) stream << "[\n";	  
    for (int i=0; i<this->moduleInstantiations.size();i++) {
      if(i > 0) stream << ",\n";	    
      this->moduleInstantiations[i]->print_python(stream, stream_def, indent+"  ", inlined, skip_brackets);
    }
    if(!inlined) stream << "\n";
    if(!skip_brackets)  stream << "]";	  
  }    
